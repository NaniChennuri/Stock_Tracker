// ── SALES (Stock Out) ─────────────────────────────────────────────────────
let saleItems = [];
let salePaymentType = 'paid'; // 'paid' | 'debit'

function renderSales() {
  const s = getState();
  const history = [...s.sales].reverse();
...
function saleForm() {
  ...
  return `
    ...
    <div id="sale-items">
      ${saleItems.map((item, i) => saleItemRow(item, i)).join('')}
    </div>
    <button class="btn-add-row" onclick="addSaleItemRow()">+ Add Item</button>

    <div class="sale-total-bar">
      Total: <span class="sale-total-val green">${fmtCurrency(total)}</span>
    </div>
...
function setSalePayment(type) {
  ...
  document.querySelectorAll('.pay-opt').forEach(el => {
    if ((type === 'paid' && !el.classList.contains('debit')) ||
        (type === 'debit' && el.classList.contains('debit'))) {
...
function onSaleQtyChange(input, i) {
  saleItems[i].qty = parseInt(input.value) || 0;
  updateSaleTotal();
}

function onSalePriceChange(input, i) {
  saleItems[i].price = parseFloat(input.value) || 0;
  updateSaleTotal();
}
...
function saveSale() {
  ...
  valid.forEach(item => {
    ...
    if (idx >= 0) s.inventory[idx].qty = Math.max(0, s.inventory[idx].qty - item.qty);
  });

  const total = valid.reduce((a, i) => a + (i.qty * i.price), 0);
  const saleId = uid();
  s.sales.push({ id: saleId, date, customer, items: valid, total, paymentType: salePaymentType });

  if (salePaymentType === 'debit') {
    if (!s.debits) s.debits = [];
    const dueDate = document.getElementById('s-duedate')?.value || '';
    s.debits.push({ id: uid(), saleId, date, customer, total, paid: 0, dueDate, payments: [] });
  }
...
function openSaleDetail(id) {
  const sale = getState().sales.find(x => x.id === id);
  if (!sale) return;
  showModal(`
    <div class="modal-header">
      <div>
